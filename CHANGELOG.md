...
- **(Minor) Copilot review round 2 / round 3 — round-trip fidelity for absent upstream headers.** The bot's round-2 review flagged the cases where Trellis.Http synthesizes typed errors with default empty/zero values that ASP's renderer then fabricates as wire headers the upstream did not send. An initial fix (renderer-side `when` guards skipping empty `Allow` / zero `Content-Range`) was added, but the round-3 review correctly pushed back: those guards overload `EquatableArray<string>.Empty` and `RangeNotSatisfiable(0)` with sentinel "absent" semantics, breaking legitimate cases (an empty representation should still round-trip as `Content-Range: bytes */0`; an explicitly-empty Allow could mean "no methods allowed"). The renderer guards were therefore **withdrawn**. The proper fix is at the typed-error level (add an "absent" slot to the relevant `Error` records), which is out of scope for this PR. The current behavior is documented as a known limitation:
  - **Token68 round-trip.** RFC 7235 also defines a `token68` form (`WWW-Authenticate: Negotiate <base64-token>`) used by SPNEGO/Negotiate/NTLM for multi-step authentication. `AuthChallenge` has no slot for the bare token, so when an upstream sends a token68-form challenge `BuildChallenge` captures only the scheme and the token is dropped on round-trip. Documented in `BuildChallenge` xmldoc and the 401 rows of both api ref and article; callers needing token68 support must use `ToResultAsync(statusMap)` or the body-aware overload to inspect headers directly.
  - **Empty-array / zero-length wire artifacts.** When an upstream 405 omits `Allow` or 416 omits `Content-Range`, the strict default mapper synthesizes the typed error with default empty/zero values, and rendering the failure back through ASP produces an empty `Allow:` header or `Content-Range: bytes */0`. This is a known limitation of the typed-error model; use `ToResultAsync(statusMap)` for non-conforming-upstream handling.

Tests: **+22** (round 1: +18; round 3 coverage: +4 — token68 documented-degradation, quoted-pair unescape, unparseable-parameter fallback, single-scheme positive). The round-2 renderer guards and tests were withdrawn after round-3 review.
