...
Tests: **+18** new tests in `Trellis.Http.Tests` covering null-`error` guards on all three `Handle*Async` methods (matched + non-matched paths); response disposal on the null-`jsonTypeInfo` path for `ReadJsonAsync` and `ReadJsonMaybeAsync`; absence of response-body content (and absence of `ex.Path`) in the deserialization-failure `Detail`; header preservation for `401` (`WWW-Authenticate` schemes + parsed parameters), `405` (`Allow`), `416` (`Content-Range` length + unit), `429` (`Retry-After: seconds`), and `503` (`Retry-After: date`); negative tests for missing `Allow` / `Retry-After` / `Content-Range` / `WWW-Authenticate` keeping the typed payloads empty/null; malformed negative `Retry-After` treated as absent.
