...
- **Public XML `<exception>` documentation gaps** — added missing `<exception cref="ArgumentNullException">` (and supporting `<param>` / `<returns>` / `<typeparam>` where missing) on:
  - `Trellis.Core.Result.Ok<TValue>(TValue)`, `Result.Fail<TValue>(Error)`, `Result.Ok()`, `Result.Fail(Error)` — the four primary `Result` factory entry points.
  - `Trellis.Core.Maybe.Optional(...)` (both overloads, which call `ArgumentNullException.ThrowIfNull(function)`).
  - `Trellis.ServiceDefaults.TrellisServiceCollectionExtensions.AddTrellis(...)` for the `services` and `configure` null checks.
  - `Trellis.Http.HttpResponseExtensions.ToResultAsync(...)` and `ReadJsonOrNoneOn404Async(...)` for the `response` and `jsonTypeInfo` null checks.
