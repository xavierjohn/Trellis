...
- **`AddTrellisActorForwarding` extension** on `IReverseProxyBuilder` registers options + validator + minter + transform provider + a hosted-lifecycle registration validator. The transform captures `ClusterConfig` at `TransformBuilderContext` build time (not at request time — `RequestTransformContext` has no `Cluster` property), and the per-request half resolves the scoped `IActorProvider`, mints a fresh JWT carrying the full `Actor` surface for the destination cluster, and overwrites the upstream `Authorization` header. **No actor on the inbound request → the upstream `Authorization` header is CLEARED** before YARP forwards the request, so the external (gateway-validated) bearer token cannot reach the downstream service. This is the fail-closed posture added in the P4 round-1 security review: without the clear, YARP's default header-copy would forward the external token to the downstream, creating a header-authority confusion vector for any downstream that hasn't pinned its audience strictly to the gateway-minted value.
- **`MapTrellisDiscoveryEndpoint` extension** on `IEndpointRouteBuilder` publishes `/.well-known/openid-configuration` + `/.well-known/jwks.json` (paths overridable) constructed from `Issuer` + `PublicBaseUrl` — never inferred from request context (`HttpRequest.Host` is spoofable behind reverse proxies). JWKS includes every key in the active rotation ring (current `SigningCredentials.Key` + `PreviousSigningKeys`). Symmetric keys are stripped before publication even though startup validation rejects them — defense in depth in case a future refactor loosens validation. Private RSA / EC components (`d`, `p`, `q`, `dp`, `dq`, `qi`, `k`) are explicitly never serialized.
- **Internal JWT contract v1** matches the consumer-side `TrellisInternalJwtActorProvider` defaults exactly: `iss`, `aud` (per-cluster), `sub` (via `ActorIdResolver`), fresh `jti` per token, `iat` / `nbf` / `exp` (lifetime capped to `[1m, 30m]`), multi-valued `permissions` + `forbidden_permissions` (never comma-joined or JSON-stringified), `trellis_actor_contract_version=1`, `trellis_permissions_count`, and **`trellis_forbidden_permissions_count` always emitted (including `"0"` for empty sets)** — the deny-overrides-allow contract integrity invariant that lets the consumer side detect a misbehaving proxy stripping the deny claims. One claim per projected attribute, claim name = entry key.
- **Security-amended options surface.** `TrellisActorForwardingOptions` enforces (1) asymmetric-only signing — symmetric keys are rejected at startup; HS256/HS384/HS512 are dropped from v1 because publishing symmetric keys in JWKS leaks the secret AND refusing to publish them silently breaks the discovery story. (2) Every `SigningCredentials` MUST include a non-empty `Kid` — startup validation rejects missing-kid keys so downstream kid-aware rotation works; the transform emits `kid` in the JWT header. (3) `Lifetime` capped to `[1m, 30m]` — short-lived tokens are the contract, the cap is defense against accidental mis-configuration. (4) `ActorIdResolver` defaults to `actor => actor.Id.Value` but consumers fronting multiple IdPs MUST override to namespace the `sub` claim (`{issuer}|{tenant}|{externalSub}`) to prevent cross-IdP subject collisions — `Actor` equality is identity-based on `Id` only. (5) `PublicBaseUrl` is required absolute (NOT inferred from `HttpRequest.Host`). (6) `PreviousSigningKeys` rotation ring rejects null entries, symmetric keys, missing-kid entries, and `kid` collisions with the active key or other previous keys.
- **Audit-log redaction by construction.** Three `[LoggerMessage]` events (token minted, no-actor skip, mint failed) carry only low-cardinality metadata: cluster id, `kid`, permission count, forbidden count, exception **type** name. NEVER the raw JWT, raw claim values, actor IDs, tenant IDs, attribute values, or exception messages (can carry secret material from the key handle). Verified by a dedicated test that hard-asserts no claim-value string appears in any log entry across the full event surface.
- **Startup composition checks.** `AddTrellisActorForwarding` registers a `TrellisActorForwardingRegistrationValidator` `IHostedLifecycleService` that fails host start if no `IActorProvider` is registered — turning a per-request `InvalidOperationException` from the DI container into a clear startup error pointing at the missing `AddClaimsActorProvider` / `AddEntraActorProvider` registration.
- **Cookbook Recipe 34** ("Microservices behind YARP, end-to-end") added to `trellis-api-cookbook.md` with the full gateway composition example, downstream pairing reference, trust-boundary documentation (**signing-key compromise = full identity spoof until key revocation propagates**), key-rotation runbook reference, emergency revocation procedure, and the explicit mTLS-environment note ("JWT is belt-and-suspenders by design when mTLS already trusts the channel; ~25 µs/request crypto overhead is negligible vs typical workload I/O; the JWT carries application-layer identity that mTLS does not; v1.1 mTLS forwarded-headers path is on the roadmap").
- **API reference doc.** New `trellis-api-yarp.md` with the full options table, behavioral notes, internal-JWT contract v1 specification, audit-log redaction contract, and cross-links to Recipe 33 + Recipe 34.

### Added — `TrellisInternalJwtActorProvider` for gateway-minted internal JWTs (P3)

- **New actor provider.** `Trellis.Asp.Authorization.TrellisInternalJwtActorProvider` hydrates the FULL `Actor` surface — id + permissions + forbidden permissions + ABAC attributes — from a verified internal-network JWT minted by a trusted gateway (typically `Trellis.Yarp` once it ships, or any third-party gateway implementing the same contract). Unlike the stock `ClaimsActorProvider` which omits forbidden permissions and attributes, the internal-JWT provider is built around a gateway-controlled claim shape where the full Actor is intentionally carried on the wire.
- **Scheme-binding (security amendment).** The provider authenticates the configured `AuthenticationScheme` via `HttpContext.AuthenticateAsync(scheme)` and reads the resulting `ClaimsPrincipal` — it NEVER trusts `HttpContext.User` directly. This is the canonical fail-closed posture for an internal-JWT contract: a misconfigured middleware planting a `ClaimsPrincipal` with matching claim names on `HttpContext.User` cannot silently translate forged claims.
- **Sentinel + count claim contract (security amendment).** The gateway MUST mint three contract claims — `trellis_actor_contract_version=1`, `trellis_permissions_count`, `trellis_forbidden_permissions_count` (even when zero) — so that a misconfigured proxy stripping the deny-claim set cannot be confused with the gateway evaluating the deny-claim set to empty. The latter is normal; the former is a privilege-escalation footgun against deny-overrides-allow. Missing / mismatched / malformed → `Maybe<Actor>.None` → HTTP 401.
- **`RequiredAttributes` (security amendment).** Attribute names that MUST be present and non-empty on every request (typical: `tenant_id`, `mfa`). Missing / empty / duplicated → fail closed. Each name must also be a key in `AttributeClaimMap`; this consistency is startup-validated.
- **`StrictClaimShape` (security amendment).** Default `true`. Permission / forbidden-permission / mapped-attribute values containing commas or starting with `[` / `{` are rejected — these shapes indicate a gateway-side bug that comma-joined a set or serialized JSON into a single claim, which would silently create one bogus value instead of N separate ones.
- **`ExpectedIssuer` / `ExpectedAudience` (security amendment).** Optional defense-in-depth runtime cross-checks complementing `JwtBearerOptions.TokenValidationParameters` configured at the `AddJwtBearer` level. The cookbook mandates the strict validation profile (`ValidateIssuer`, `ValidateAudience`, `ValidateLifetime`, `RequireSignedTokens`, `ValidAlgorithms`, tight `ClockSkew`); the in-provider checks are safety nets for consumers who haven't followed the strict profile yet.
- **Reserved JWT claim name guard (security amendment).** Startup validation rejects reserved JWT claim names (`iss`, `aud`, `exp`, `nbf`, `iat`, `jti`, `sub`) as permission / forbidden-permission / attribute claim sources. Escape hatch via `UnsafeAllowRegisteredClaimNames = true`.
- **Logging redaction (security amendment).** All failure-path log entries carry low-cardinality metadata only — scheme name, claim TYPE names, counts, and consumer-configured literal expected values. The provider NEVER logs the JWT body, raw claim values, actor IDs, tenant IDs, or other PII. Verified by a dedicated test that iterates all failure modes against a set of sensitive-substring assertions.
- **Registration helpers.** New `services.AddTrellisInternalJwtActorProvider(Action<TrellisInternalJwtActorOptions>? configure = null)` (in `Trellis.Asp.Authorization`) and matching `TrellisServiceBuilder.UseTrellisInternalJwtActor(Action<TrellisInternalJwtActorOptions>? configure = null)` builder slot (in `Trellis.ServiceDefaults`). Mutually exclusive with the other actor-provider selectors. Startup-validates options via `IValidateOptions<TrellisInternalJwtActorOptions>` + `ValidateOnStart()`.
- **Reference docs.** Type docs for `TrellisInternalJwtActorProvider` and `TrellisInternalJwtActorOptions` in `trellis-api-asp.md`; new builder-slot row in `trellis-api-servicedefaults.md`; Path B snippet appended to cookbook Recipe 7 ("Microservices") showing the strict `AddJwtBearer` validation profile + composition-root example. [Recipe 33](docs/docfx_project/api_reference/trellis-api-cookbook.md#recipe-33--strict-addjwtbearer-validation-profile-for-usetrellisinternaljwtactor) (default JWKS-discovery + air-gapped static-key-ring profiles + key-rotation runbook + logging-redaction checklist + tenant-isolation defense-in-depth pattern) ships in P3.5.
