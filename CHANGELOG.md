...
Trellis now publishes one `Meter`, `Trellis.Validation`, with a counter incremented once per violation as a `FieldViolation` or `RuleViolation` is created, tagged `validation.code` and `validation.violation`. It exists to answer one question the rest of the telemetry cannot: *is this rule dead?* A trace answers it only for sampled requests, so a reason code with zero volume is indistinguishable from one whose traces were all sampled away — which matters for a frozen vocabulary, where a code that no producer reaches is a defect nobody can see.
