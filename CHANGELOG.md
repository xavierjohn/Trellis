...
- **(Minor) Copilot review round 2 / round 3 / round 4 / round 5 — round-trip fidelity for absent or unusable upstream headers.** The bot iterated 4 times on the typed-error-vs-wire-header round-trip problem. The final converged design:
  ...
  - **Documented limitation: token68 round-trip.** RFC 7235 also defines a `token68` form (`WWW-Authenticate: Negotiate <base64-token>`) used by SPNEGO/Negotiate/NTLM for multi-step authentication. `AuthChallenge` has no slot for the bare token, so when an upstream sends a token68-form challenge `BuildChallenge` captures only the scheme and the token is dropped on round-trip. Documented in `BuildChallenge` xmldoc and the 401 rows of both api ref and article; callers needing token68 support must use the body-aware `ToResultAsync(mapper, ct)` overload — the only public API in this package that exposes `HttpResponseMessage.Headers` for direct inspection.

Tests: **+28** total on the branch vs `main` (round 1: +18; round 3 coverage: +4; round 4: +2 — null-error disposal for `Handle*Async` (consolidated assertions across the three methods); 405 without `Allow` falls through; 416 without `Content-Range` falls through; round 5: +4 — 416 with `bytes */0` produces typed `RangeNotSatisfiable(0, "bytes")` for empty resource; 416 with `bytes 0-99/*` (no length) falls through; 405 with literal-empty `Allow:` value falls through; 429 with `Retry-After: 0` preserves zero delta — these close gaps in initial i-H2 coverage that motivated the user's "test cases that verify the expected RFC responses" feedback). The round-2 renderer guards and round-3 misleading empty-scheme test were withdrawn.
