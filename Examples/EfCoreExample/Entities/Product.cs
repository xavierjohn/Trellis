...
public class Product : Entity<ProductId>
{
    ...
    public static Result<Product> TryCreate(string? name, decimal price, int stockQuantity) =>
        ProductName.TryCreate(name, nameof(name))
            .Ensure(_ => price > 0, _ => Error.InvalidInput.ForField(field: nameof(price), code: ValidationCodes.ValueGreaterThan, detail: "Price must be greater than zero"))
            .Ensure(_ => stockQuantity >= 0, _ => Error.InvalidInput.ForField(field: nameof(stockQuantity), code: ValidationCodes.ValueGreaterThanOrEqual, detail: "Stock cannot be negative"))
