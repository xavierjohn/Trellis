...
public class Product : Entity<ProductId>
{
    ...
    public static Result<Product> TryCreate(string? name, decimal price, int stockQuantity) =>
        ProductName.TryCreate(name, nameof(name))
            .Ensure(_ => price > 0, _ => Error.InvalidInput.ForField(nameof(price), ValidationCodes.ValueGreaterThan, "Price must be greater than zero"))
            .Ensure(_ => stockQuantity >= 0, _ => Error.InvalidInput.ForField(nameof(stockQuantity), ValidationCodes.ValueGreaterThanOrEqual, "Stock cannot be negative"))
    ...
    public Result<Product> ReduceStock(int quantity) =>
        this.ToResult()
            ...
            .Ensure(_ => StockQuantity >= quantity, _ => Error.InvalidInput.ForField(nameof(quantity), "product.insufficient-stock", $"Insufficient stock. Available: {StockQuantity}"))
