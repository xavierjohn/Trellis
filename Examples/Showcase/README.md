...
| **IETF Idempotency-Key middleware** (opt-in `[Idempotent]` / `.WithMetadata(new IdempotentAttribute())` on POST — first call executes; same-scope, same-key, matching-fingerprint retry replays the unexpired snapshot with `Idempotent-Replayed: true`; a different request fingerprint against an existing reservation or unexpired snapshot is rejected as 422, including changed method, path, query, fingerprinted headers, or body; missing key on an opted-in endpoint is rejected as 400) | `Showcase.{Mvc,MinimalApi}/Program.cs`, `Showcase.MinimalApi/Endpoints/TransferEndpoints.cs`, `Showcase.Mvc/Controllers/TransfersController.cs` |
