...
public class ScalarValueTypeHelperTests
{
    ...
    public class ValidVO : ScalarValueObject<ValidVO, string>, IScalarValue<ValidVO, string>
    {
        ...
        public static Result<ValidVO> TryCreate(string? value, string? fieldName = null) =>
            string.IsNullOrEmpty(value) ? Result.Fail<ValidVO>(new Error.InvalidInput(EquatableArray.Create(new FieldViolation(InputPointer.ForProperty(fieldName ?? "field"), ValidationCodes.Unspecified) { Detail = "Required" }))) : Result.Ok(new ValidVO(value));
    }
    ...
    public class InterfaceOnlyValidated : IScalarValue<InterfaceOnlyValidated, int>
    {
        ...
        public static Result<InterfaceOnlyValidated> TryCreate(int value, string? fieldName = null) =>
            value > 0 ? Result.Ok(new InterfaceOnlyValidated(value)) : Result.Fail<InterfaceOnlyValidated>(new Error.InvalidInput(EquatableArray.Create(new FieldViolation(InputPointer.ForProperty(fieldName ?? "field"), ValidationCodes.Unspecified) { Detail = "Must be positive." })));
        ...
        where T : IComparable
    {
        ...
        public static Result<GenericVO<T>> TryCreate(T? value, string? fieldName = null) =>
            value is null ? Result.Fail<Asp.Tests.ScalarValueTypeHelperTests.GenericVO<T>>(new Error.InvalidInput(EquatableArray.Create(new FieldViolation(InputPointer.ForProperty(fieldName ?? "field"), ValidationCodes.Unspecified) { Detail = "Required" }))) : Result.Ok(new GenericVO<T>(value));
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1000:Do not declare static members on generic types", Justification = "Required by IScalarValue interface pattern")]
        ...
        IComparable<MultiInterfaceVO>
    {
        ...
        public static Result<MultiInterfaceVO> TryCreate(string? value, string? fieldName = null) =>
            string.IsNullOrEmpty(value) ? Result.Fail<MultiInterfaceVO>(new Error.InvalidInput(EquatableArray.Create(new FieldViolation(InputPointer.ForProperty(fieldName ?? "field"), ValidationCodes.Unspecified) { Detail = "Required" }))) : Result.Ok(new MultiInterfaceVO(value));
