...
/// and the collection selector returns a <see cref="ValueTask{T}"/> of <see cref="Maybe{T}"/>.
/// </summary>
[DebuggerStepThrough]
public static class MaybeLinqExtensionsValueTaskRightAsync
{
    /// <summary>
    /// Projects a synchronous <see cref="Maybe{T}"/> through an async collection selector
    /// (LINQ <c>SelectMany</c>: sync source / async continuation, ValueTask variant).
    /// </summary>
