...
- **Safe to scale out** — each drain claims its batch under an optimistic lease (`LockedBy` is a concurrency token), so multiple relay instances never double-publish and a drain that outlives its lease abandons its write instead of clobbering the new owner — no leader election needed.
