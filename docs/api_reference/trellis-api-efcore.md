...
| `public static ModelConfigurationBuilder ApplyTrellisConventionsCore(this ModelConfigurationBuilder configurationBuilder, IEnumerable<(Type ClrType, Type ProviderType)> scalars, IEnumerable<Type> composites)` | `ModelConfigurationBuilder` | Low-level helper consumed by both the reflection-based `ApplyTrellisConventions` overload and the source-generated `ApplyTrellisConventionsFor<TContext>`. Registers the supplied scalar converters and composite types, then adds the same fixed Trellis conventions. |

### `GeneratedTrellisConventions`

```csharp
public static class GeneratedTrellisConventions
```

Source-generated by `ApplyTrellisConventionsForGenerator` in the **consuming** project (a class with this name does not exist in the package itself). Emitted whenever the consuming compilation contains at least one concrete `DbContext`-derived type with at-least-internal accessibility.

| Signature | Returns | Description |
| --- | --- | --- |
| `public static ModelConfigurationBuilder ApplyTrellisConventionsFor<TContext>(this ModelConfigurationBuilder configurationBuilder) where TContext : DbContext` | `ModelConfigurationBuilder` | Reflection-free, AOT-clean alternative to `ApplyTrellisConventions(Assembly[])`. Walks `TContext`'s `DbSet<>` properties and reachable property graph at compile time, then delegates to `ApplyTrellisConventionsCore` with the discovered scalars and composites. Throws `InvalidOperationException` at runtime when called for a `TContext` that has no generated body. |
