...
**Implementation.** A static `AsyncLocal<Stack<TResource>?>` per closed `(TMessage, TResource)` pair, with **copy-on-push / restore-on-dispose** semantics. The pipeline behavior pushes inside a `using` block around `await next(...)` only after `Authorize` returns success — denied authorizations cannot expose the loaded resource via the accessor. Copy-on-push (not mutate-in-place) is the load-bearing correctness piece for nested `mediator.Send` and concurrent `Task.WhenAll` dispatch of the same closed pair; mutating a shared `Stack<T>` would cross-contaminate sibling forks because `AsyncLocal` flows the value *reference* into child execution contexts. (GPT-5.5 review round 2, blocking finding 1.)
