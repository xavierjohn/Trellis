...
>
> **Source of truth.** This document is the authoritative v2 plan. Anything in this repo (api_reference docs, csproj layout, package list in `README.md`, `AGENTS.md`) that contradicts this doc is the bug. **Read §2 (Proposed Package Map) and §15 (Phasing) before starting any v2 work.**
>
...
template/
  ...
  .github/       — copilot-instructions.md + 13 trellis-api-*.md files (ship in current template)
```
...
9. **Template currently ships its own `.github/copilot-instructions.md` and 13 `trellis-api-*.md` files.** → **Migrated in Phase 6 to a root `AGENTS.md` as the canonical AI surface.** The repo-level `docs/docfx_project/api_reference/` files are the source; the template's copies are kept in sync by Phase 5 tooling.
...
- **TDD is mandatory** per `TrellisFramework/AGENTS.md`. Every Phase 1a–7 PR follows red→green→refactor: failing test(s) committed first (or in the same PR with a clear "test first" commit ordering), then the implementation that satisfies them, then the refactor. The pre-submission checklist in `AGENTS.md` (build clean, all tests green, public API surface reviewed, analyzers clean, docs updated) is enforced on every PR — no "we'll backfill tests later" exceptions during the rewrite.
...
| **Layering clarity (Clean Arch)** | 7 | 8 | +1 | Already strong (template proves it). New: explicit layer-reference rules canonized in `AGENTS.md`. |
