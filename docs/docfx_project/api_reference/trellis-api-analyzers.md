...
types: [TrellisDiagnosticIds, DiagnosticDescriptors, ResultNotHandledAnalyzer, UseBindInsteadOfMapAnalyzer, UnsafeValueAccessAnalyzer, ResultDoubleWrappingAnalyzer, AsyncResultMisuseAnalyzer, MaybeDoubleWrappingAnalyzer, UseResultCombineAnalyzer, AsyncLambdaWithSyncMethodAnalyzer, ThrowInResultChainAnalyzer, UnsafeValueInLinqAnalyzer, CombineLimitAnalyzer, UseSaveChangesResultAnalyzer, HasIndexMaybePropertyAnalyzer, UnsafeResultDeconstructionAnalyzer, DefaultResultOrMaybeAnalyzer, CompositeValueObjectDtoConverterAnalyzer, RedundantEfConfigurationAnalyzer, OwnedEntityInitOnlyPropertyAnalyzer, MediatorUnitInResultAnalyzer, AddResultGuardCodeFixProvider, UseBindInsteadOfMapCodeFixProvider, UseAsyncMethodVariantCodeFixProvider, UseSaveChangesResultCodeFixProvider, TRLS043, TRLS044, TRLS045, TRLS054, TRLS055, TRLS057, TRLS058, TRLS059]
version: v3
...
| `TRLS059` | Warning | `Result` wraps `Mediator.Unit` instead of `Trellis.Unit` | Flags `Result<Mediator.Unit>` (the martinothamar/Mediator `Unit`). Trellis and Mediator both define a `Unit`; in a handler file with a file-scoped `using Mediator;`, a bare `Unit` binds to `Mediator.Unit` (the nearer `using` wins, no `CS0104`), so `Result<Unit>` silently compiles as `Result<Mediator.Unit>`. The Trellis.Asp response layer maps only `Result<Trellis.Unit>` to `204 No Content`, so the endpoint returns `200` and only an HTTP integration test catches it. Matches every `Result<Mediator.Unit>` occurrence — handler return types, `ICommand`/`IQuery` and handler-interface declarations, and locals — but never `Mediator.Unit` outside `Result<>` (e.g. `ICommand<Mediator.Unit>`, the legitimate void-message marker). Fix: qualify as `Result<Trellis.Unit>` or add a file-scoped `using Unit = Trellis.Unit;`. |
