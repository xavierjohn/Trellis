...
Default handler exceptions are still **logged and swallowed** by `MediatorDomainEventPublisher`; cascade detection only catches handler-raised events. Durable side effects and durable at-least-once retry require the transactional outbox, which **is shipped** in `Trellis.EntityFrameworkCore.Outbox` (`AddTrellisOutbox`, `OutboxCaptureInterceptor`, `OutboxRelay`) — see [trellis-api-efcore-outbox.md](trellis-api-efcore-outbox.md#use-this-file-when).
