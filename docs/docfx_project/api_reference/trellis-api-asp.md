...
| `CurrentPropertyName` | `string?` (get/set) | Async-local property name for the property currently being deserialized. Set by `PropertyNameAwareConverter<,>` (reflection mode) and read by both the reflection-mode `ScalarValueJsonConverterBase<,,>` and the AOT-generated converter (which falls back to a camel-cased type name when this is `null`). |
