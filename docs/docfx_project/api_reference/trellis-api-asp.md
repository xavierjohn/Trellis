...
| Signature | Returns | Description |
| --- | --- | --- |
| `public static void RegisterProperty<T>()` | `void` | Registers a factory producing `PathTrackingPropertyConverter<T>` for a scalar property, so Native AOT validation uses the effective JSON property name supplied at runtime. Idempotent. |
