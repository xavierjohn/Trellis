...
- **Responses are JSON-only, and XML output formatters are not supported.** Trellis always writes JSON, but success and failure reach that outcome by different routes — and only one of them is JSON-only by construction. The distinction matters because it is what makes one XML formatter harmless and another fatal.
  - *Success* responses bypass negotiation entirely. `ToHttpResponse(...)` produces an `IResult` that writes its own body, so a request sending `Accept: application/xml` against a Trellis endpoint receives `application/json` even when an XML formatter is registered and the body type is XML-serializable. The same action returning a plain `Ok(dto)` would return `application/xml`.
  - *Failure* responses do reach the formatters, because `ResponseFailureWriter` writes through `Results.Problem(...)` / `Results.ValidationProblem(...)` and therefore `IProblemDetailsService`. With `AddXmlSerializerFormatters()` this is harmless — `XmlSerializerOutputFormatter` declines `ProblemDetails` and MVC falls back to `application/problem+json`. With `AddXmlDataContractSerializerFormatters()` it is **not**: Trellis carries `FieldViolationProblemDetail[]` and nested maps in `ProblemDetails.Extensions`, which is typed `object?`, and `DataContractSerializer` refuses values whose runtime type it was not given via `KnownTypeAttribute`. The write throws `SerializationException` and the client gets a `500` instead of the problem document.
  - **Known limitation, tracked for a post-alpha fix.** Do not register `AddXmlDataContractSerializerFormatters()` in an application that uses Trellis response mapping. Note also that the `application/problem+xml` entry that `ProblemDetailsActionResult` declares in its content types exists to match what MVC's `ObjectResultExecutor` infers, so that a `[Produces]` attribute cannot overwrite the list — it is not a promise that Trellis can render XML.
