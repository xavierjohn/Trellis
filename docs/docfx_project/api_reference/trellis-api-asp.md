...
- **Responses are JSON-only, and XML output formatters are not supported.** Trellis never participates in XML content negotiation, and registering an XML output formatter alongside Trellis is unsupported in this release.
  ...
  - **Known limitation, tracked for a post-alpha fix.** Do not register `AddXmlDataContractSerializerFormatters()` in an application that uses Trellis response mapping. The `application/problem+xml` entry `ProblemDetailsActionResult` declares in its content types exists to match what MVC's `ObjectResultExecutor` infers, so that a `[Produces]` attribute cannot overwrite the list — it is not a promise that Trellis can render XML.
