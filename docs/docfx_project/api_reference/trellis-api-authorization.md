...
**Silent-403 diagnostics.** When `ClaimsActorProvider` resolves zero permissions on an authenticated identity that does carry other claims, it emits one warning per application lifetime (`EventId 2`). When the configured `PermissionsClaim` resolves to a single value that parses as a JSON object or array, it emits one error per application lifetime (`EventId 3`) recommending `NestedJsonPathClaimsActorProvider`. **Silent-401 diagnostic.** When the configured `ActorIdClaim` (and its short↔long fallback) resolves nothing on an authenticated identity that carries other claims, it emits one warning per application lifetime (`EventId 5`) naming the configured claim and the claim types actually present. Set `ClaimsActorOptions.ValidateClaimShapeOnFirstUse = false` to suppress all three. Custom providers deriving from `ClaimsActorProvider` that pass `logger: null` to the base constructor (because they own a logger of their own category, as `NestedJsonPathClaimsActorProvider` does) should override `protected virtual ILogger? DiagnosticLogger` to return their own logger; otherwise diagnostics raised on resolution paths that delegate back to the base are silently dropped.
...
| Signature | Returns | Description |
| --- | --- | --- |
| `Task<Maybe<Actor>> GetCurrentActorAsync(CancellationToken cancellationToken = default)` | `Task<Maybe<Actor>>` | Returns the current authenticated actor wrapped in `Maybe.From(...)`, or `Maybe<Actor>.None` when the request has no usable authenticated actor. The mediator authorization pipeline maps `Maybe.None` to `Error.AuthenticationRequired` (HTTP 401) per RFC 9110 §15.5.2. Implementations should throw `InvalidOperationException` only for genuine infrastructure or configuration failures (no `HttpContext`, mapping delegate threw, etc.) — those still surface as `Error.Unexpected` (HTTP 500). "No actor" is client-error state, not an exception. Register as scoped. |
