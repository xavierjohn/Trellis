...
| Goal | Use this | See |
|---|---|---|
| Filter an `IQueryable<T>` by a `Maybe<TInner>` property (`<`, `<=`, `>`, `>=`, string methods, presence, equality) | `WhereHasValue(x => x.M, value => value < cutoff)` / `WhereHasValue(x => x.M)` / `WhereEquals` / `WhereNone` | [`MaybeQueryableExtensions`](#maybequeryableextensions) |
...
| Ad-hoc EF `IQueryable<T>` filtering or ordering | Prefer `WhereHasValue(x => x.M, value => value < cutoff)`, `WhereHasValue(x => x.M)`, `WhereNone`, `WhereEquals`, `OrderByMaybe`, etc. | These helpers target the mapped storage member directly and do not require the `MaybeQueryInterceptor` for that specific predicate. Scalar value-object `.Value` access still requires `ScalarValueQueryInterceptor` (included in `AddTrellisInterceptors()`). |
...
| `public static Task<Result<int>> SaveChangesResultAsync(this DbContext context, bool acceptAllChangesOnSuccess, CancellationToken cancellationToken = default)` | `Task<Result<int>>` | Wraps `SaveChangesAsync`; maps `DbUpdateConcurrencyException` to `new Error.Conflict(null, FaultCodes.ConcurrentModification)`, duplicate-key `DbUpdateException` to `new Error.Conflict(null, "duplicate.key")`, and foreign-key `DbUpdateException` to `new Error.Conflict(null, "referential.integrity")`. |
...
**Typed predicates, not inferred comparison capabilities.** Use `WhereHasValue(propertySelector, predicate)` for comparisons and other value conditions. The predicate is an `Expression<Func<TInner, bool>>`: C# validates its operators and methods against the actual inner type. No `IComparable<TInner>` or generic-math constraint is required. Trellis inlines the expression against the nullable storage member and adds an `IS NOT NULL` guard. Even constant-true predicates and reference-type inequalities exclude absent values.

**Provider contract.** A valid C# expression does not guarantee SQL translation. Numeric/date operators, string methods, GUID comparisons, and custom value-object expressions must be supported by the configured provider. For example, SQLite translates `int` and `DateTime` relational predicates but rejects a `DateTimeOffset` relational predicate; SQL Server translates the latter. String comparisons follow database collation, not necessarily CLR culture/ordering. GUID ordering can also differ across providers. Choose an explicit comparison expression that matches the database ordering instead of assuming `IComparable<T>` provides that guarantee. Translation failures propagate; the helper never compiles the predicate or filters on the client. Scalar value-object `.Value` access still needs `AddTrellisInterceptors()`; a value converter alone does not promise arbitrary method translation.

**Breaking migration.** The four operator-specific comparison helpers have been removed. Move the comparison into the second lambda; use `<`, `<=`, `>`, or `>=` there. Presence, equality, and ordering overloads are unchanged. All selectors must remain direct `Maybe<TInner>` property accesses; a null source, selector, or predicate throws `ArgumentNullException`, and an unsupported selector shape throws `ArgumentException`.

```csharp
var overdue = orders.WhereHasValue(o => o.SubmittedAt, submittedAt => submittedAt < cutoff);

// Reusable expression variables are supported; compiled Func<TInner, bool> delegates are not.
Expression<Func<DateTime, bool>> inWindow = value => value >= start && value < end;
var recent = orders.WhereHasValue(o => o.SubmittedAt, inWindow);
```

For a shared domain specification evaluated both in memory and by EF, keep using the natural `o.SubmittedAt.HasValueWhere(value => value < cutoff)` shape with `AddTrellisInterceptors()`. The explicit query helper does not replace that Core API and does not require the Maybe interceptor for its own storage access.
...
| `public static IQueryable<TEntity> WhereHasValue<TEntity, TInner>(this IQueryable<TEntity> source, Expression<Func<TEntity, Maybe<TInner>>> propertySelector, Expression<Func<TInner, bool>> predicate) where TEntity : class where TInner : notnull` | `IQueryable<TEntity>` | Filters to present values satisfying the typed predicate. Accepts inline lambdas or reusable expression trees, never compiled delegates. |
