...
- **Composite value objects need a parameterless constructor for EF Core materialization.** `[OwnedEntity]` (on a `partial` class with `{ get; private set; }` properties) generates a private one. `[OwnedEntity]` is **optional**: composite value objects are registered as owned types by convention (from `ValueObject` inheritance), not by the attribute — the attribute exists *only* to generate the parameterless constructor. So you have two equivalent options: annotate with `[OwnedEntity]`, **or** declare a private parameterless constructor yourself (as `Money` does). A mapped composite value object with neither fails fast at model-build with an actionable `TrellisPersistenceMappingException` naming the type — replacing EF Core's cryptic "No suitable constructor was found".
  - **`[OwnedEntity]` is convenient but not EF-free; the hand-written constructor is.** The attribute lives in `Trellis.EntityFrameworkCore`, so any project that applies it references that package — and EF Core flows in transitively (the compiled assembly lists `Trellis.EntityFrameworkCore`, whose dependency closure includes `Microsoft.EntityFrameworkCore`). Referencing the package **analyzer-only** (`OutputItemType="Analyzer" ReferenceOutputAssembly="false"`) is the right way to pull *just the bundled generators* — the `[OwnedEntity]` constructor generator and the `partial Maybe<T>` generator — into a domain layer, and is required if that layer uses `partial Maybe<T>` properties. But it does **not** make the layer EF-free: analyzer-only metadata does not remove `Trellis.EntityFrameworkCore` (or `Microsoft.EntityFrameworkCore`) from the dependency closure. For a domain value object that must stay genuinely free of any EF Core dependency, declare the `private` parameterless constructor by hand (as `Money` does) — that needs no reference to `Trellis.EntityFrameworkCore` at all.
