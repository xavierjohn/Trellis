...
from `Trellis.Authorization` when actor presence is an invariant. This throwing accessor does
not replace the behavior's `Maybe.None` to `Error.AuthenticationRequired` handling, check
permissions, or cache independently. Missing actor state inside that explicitly trusted
context is a fault; ordinary unauthenticated requests must still take the normal 401 path.
