...
```csharp
public sealed class TrackedAggregateDomainEventDispatchBehavior<TMessage, TResponse>
...
2. **`TracingBehavior<,>`** — opens an OpenTelemetry `Activity` per message under the `"Trellis.Mediator"` activity source. On failure tags `error.code` / `error.type` and sets `ActivityStatusCode.Error`; thrown exceptions also add the standard exception event (`exception.type`, `exception.message`, `exception.stacktrace`). `Error.Detail` is redacted from `StatusDescription` unless `TrellisMediatorTelemetryOptions.IncludeErrorDetail` is `true`.
