...
| Register an actor provider | `.UseClaimsActorProvider()`, `.UseNestedJsonPathClaimsActorProvider()`, `.UseEntraActorProvider()`, `.UseDevelopmentActorProvider()`, or `.UseTrellisInternalJwtActor()` (for gateway-minted internal JWTs) | [`TrellisServiceBuilder`](#trellisservicebuilder), [ASP actor providers](trellis-api-asp.md#namespace-trellisaspauthorization) |
...
| `public TrellisServiceBuilder UseTrellisInternalJwtActor(Action<TrellisInternalJwtActorOptions>? configure = null)` | `TrellisServiceBuilder` | Registers `TrellisInternalJwtActorProvider` as `IActorProvider` via `AddTrellisInternalJwtActorProvider(...)`. For microservices that consume internal-network JWTs minted by a trusted gateway (typically `Trellis.Yarp` or an equivalent third-party gateway implementing the same sentinel + count claim contract); hydrates the FULL `Actor` surface including `ForbiddenPermissions` and ABAC `Attributes`. Startup-validates options via `IValidateOptions<TrellisInternalJwtActorOptions>` + `ValidateOnStart()`. Mutually exclusive with the other actor-provider selectors; a second actor-provider selector throws `InvalidOperationException`. See [`TrellisInternalJwtActorOptions`](trellis-api-asp.md#trellisinternaljwtactoroptions) for the full options surface and contract semantics. |
