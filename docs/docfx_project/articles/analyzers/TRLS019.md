...
`default(Result)` and `default(Result<T>)` are typed **failures** carrying the shared `new Error.Unexpected("default-initialized")` sentinel. They are observationally equivalent to `Result.Fail(sentinel)` / `Result.Fail<T>(sentinel)`. The explicit `default` literal at a call site obscures intent — readers see `default` and assume "the zero value of this type", but the runtime semantics are *failure*.
...
{
    if (id == Guid.Empty)
        return Result.Fail<User>(Error.InvalidInput.ForField(field: "id", code: ValidationCodes.ValueNotDefault, detail: "id is required"));
    return _repo.Get(id);
}
