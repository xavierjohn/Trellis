...
- **Concurrency exceptions bypass `shouldRetry`.** `DbUpdateConcurrencyException` is mapped immediately to `new Error.Conflict(null, "concurrent_modification")`; regenerating a natural key cannot resolve a stale rowversion.
