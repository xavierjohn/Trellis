...
The snippet project references `Trellis.Analyzers` as an analyzer, so those settings include Trellis's own rules and not merely the compiler's. A reader who copies a recipe gets the same diagnostics the recipe was compiled under, which is the only way a recipe can promise it is safe to lift: a snippet that trips TRLS003 hands the reader a build error on their first build, and the recipe is the last place that should happen. The reference is deliberately `ReferenceOutputAssembly="false"` — the snippets consume the diagnostics, not the analyzer assembly.
